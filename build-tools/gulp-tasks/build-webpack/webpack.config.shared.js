...
const boltconfig = explorer.load();

console.log(boltconfig);
...
const resolveBoltPackages = function(
  fileExt = [
    '*'
  ] // Optionally filter packages based on the file types served by the package
){
  const boltPkgsFound = {};
  const fileExts = fileExt.join(',');

  const globbedPaths = [
    path.normalize(`./src/_patterns/**/*.{${fileExts}}`),
    path.normalize(`./packages/**/*.{${fileExts}}`)
  ];
  globbedPaths.push(path.normalize(`!src/_patterns/**/node_modules/**/*`));
  // '!./src/_patterns/**/node_modules/**/*');

  globby.sync(globbedPaths).map(function (filePath) {

    const package = readPkgUp.sync({
      cwd: filePath
    });

    if (package.pkg.private === true){
      return;
    }

    // If the package name has the bolt namespace, store it in the object mapping up namespaces w/ folder roots.
    // Note: since this is an object (vs an array) duplicates aren't really as big of an issue here.
    if (package.pkg.name.includes('@bolt')) {

      boltPkgsFound[package.pkg.name] = path.resolve(__dirname, path.dirname(package.path));
    }
  });
  return boltPkgsFound;
};

// .js param not needed - just a little reference
const allBoltPkgs = resolveBoltPackages([
  'js', 'scss'
]);
...
const defaultConfig = {
  ...
  resolve: {
    ...
    alias: allBoltPkgs
    // modules: [
    //   path.resolve(__dirname, "src/_patterns/02-components/bolt-button"),
    //   "node_modules"
    // ]

    // alias: {
    //   '@bolt/package': path.join(__dirname, '../package/dist/bundle.js')
    // }
    ...
    //   // path.resolve(__dirname, 'src/_patterns/01-core'),
    //   // path.resolve(__dirname, 'src/_patterns/02-components'),
    //   'node_modules'
    // ]
  },
