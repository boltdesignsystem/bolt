const aggregateData = (generatorType, configObject, actionData, nestingNumber)  => {
  ...
  if (aggData.isTest) {
    aggData[`${generatorType}`].dir = `packages/generators/tmp/packages/${generatorType}s`;
    aggData[`${generatorType}`].patternLab = `packages/generators/tmp/docs-site/src/pages/pattern-lab/_patterns/${nestingNumber}-${generatorType}s`;
