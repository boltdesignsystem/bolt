...
const schema = readYamlFileSync(join(__dirname, '../carousel.schema.yml'));
...
describe('carousel', async () => {
  ...
  test('Default <bolt-carousel> w/o Shadow DOM renders', async function() {
    const renderedComponentHTML = await page.evaluate(() => {
      const component = document.createElement('bolt-carousel');
      btn.textContent = 'carousel Test';
  ...
  test('Default <bolt-carousel> with Shadow DOM renders', async function() {
    const defaultComponentShadowRoot = await page.evaluate(() => {
      const component = document.createElement('bolt-carousel');
      component.textContent = 'carousel Component Test -- Shadow Root HTML';
    ...
    const defaultComponentOuter = await page.evaluate(() => {
      const component = document.createElement('bolt-carousel');
      component.textContent = 'carousel Component Test -- Outer HTML';
      document.body.appendChild(component);
      component.updated();
      return component.outerHTML;
    });
    ...
    expect(renderedHTML.textContent).toEqual(
      'carousel Component Test -- Outer HTML',
    );
