...
describe('Calculator', () => {
  ...
  test('basic usage', async () => {
    const results = await render('@bolt-components-calculator/calculator.twig');
  ...
  test('adds class via Drupal Attributes', async () => {
    const results = await render(
      '@bolt-components-calculator/calculator.twig',
      {
        attributes: {
  ...
  test('renders with Shadow DOM', async function() {
    const shadowRoot = await page.evaluate(async () => {
      document.body.insertAdjacentHTML(
        'beforeend',
        '<bolt-calculator>Calculator test</bolt-calculator>',
      );
      const el = document.querySelector('bolt-calculator');
  ...
  test('renders without Shadow DOM', async function() {
    const outerHTML = await page.evaluate(async () => {
      document.body.insertAdjacentHTML(
        'beforeend',
        '<bolt-calculator no-shadow>Calculator test</bolt-calculator>',
      );
      const el = document.querySelector('bolt-calculator');
      await el.updateComplete;
      return el.outerHTML;
