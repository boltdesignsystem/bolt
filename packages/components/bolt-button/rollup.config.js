...
const componentName = pkg.name.replace('@bolt/components-', '');
...
function manifestPlugin() {
  return {
    ...
    generateBundle(options, bundle) {
      ...
      this.emitFile({
        type: 'asset',
        fileName: `${componentName ? `${componentName}.` : ''}manifest.json`,
...
function modulepreloadPlugin() {
  return {
    ...
    generateBundle(options, bundle) {
      ...
      this.emitFile({
        type: 'asset',
        fileName: `${componentName ? `${componentName}.` : ''}modulepreload.json`,
...
const themifyConfig = deepmerge(themifyOptions, {
  fallback: {
    jsonPath: path.resolve(process.cwd(), `dist/data/theming-css-vars.json`),
    ...
    sass({
      options: {
        ...
        functions: sassExportData({
          path: path.resolve(process.cwd(), 'dist/data'),
        }),
      ...
      processor: css =>
        postcss([
          ...
          .process(css)
          .then(result => {
            fs.writeFileSync('dist/bolt-button.css', result.css);
            return result.css;
          }),
    }),
    babel({
