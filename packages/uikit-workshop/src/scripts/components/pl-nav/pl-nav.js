    ...
    <button
      ...
      <span class="pl-c-nav__link-icon">
        <PlusIcon
          height={14}
          width={14}
          className={`pl-c-nav__icon--closed`}
          viewBox="0 0 24 24"
          fill="currentColor"
        />
        <MinusIcon
          height={14}
          width={14}
          className={`pl-c-nav__icon--opened`}
          viewBox="0 0 24 24"
          fill="currentColor"
        />
      </span>
...
class Nav extends BaseComponent {
  ...
  handleURLChange() {
    ...
    if (activeLink) {
      ...
      if (activeLink.classList.contains('pl-js-link-overview')) {
        ...
        const childDropdownWrapper = activeLink.parentNode.nextSibling;
        const childDropdown = childDropdownWrapper.firstChild;

        if (childDropdownWrapper && shouldAutoOpenNav) {
          if (childDropdownWrapper.tagName) {
            childDropdownWrapper.classList.add('pl-is-active');
            this.previousActiveLinks.push(childDropdownWrapper);
          }
        }
  ...
  toggleSpecialNavPanel(e) {
    ...
    if (target.classList.contains('pl-is-active')) {
      ...
      panel.classList.remove('pl-is-active');
      panel.firstChild.classList.remove('pl-is-active');
    } else {
      target.classList.add('pl-is-active');
      panel.classList.add('pl-is-active');
      panel.firstChild.classList.add('pl-is-active');
    }
  }
        ...
        {patternTypes.map((item, i) => {
          ...
          return (
            <li className={classes}>
              ...
              <div
                className={`pl-c-nav__sublist--dropdown-wrapper pl-js-acc-panel`}>
                <ol
