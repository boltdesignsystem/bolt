...
function PrerenderSPAPlugin(...args) {
  ...
  if (args.length === 1) {
    ...
    args.forEach(arg => {
      if (typeof arg === 'string') staticDir = arg;else if (Array.isArray(arg)) routes = arg;else if (typeof arg === 'object') this._options = arg;
    });
...
PrerenderSPAPlugin.prototype.apply = function (compiler) {
  const compilerFS = compiler.outputFileSystem; // From https://github.com/ahmadnassri/mkdirp-promise/blob/master/lib/index.js
  ...
  const afterEmit = (compilation, done) => {
    ...
    .then(renderedRoutes => this._options.postProcessHtml ? renderedRoutes.map(renderedRoute => {
    ...
    }) : renderedRoutes) // Run postProcess hooks.
    .then(renderedRoutes => this._options.postProcess ? Promise.all(renderedRoutes.map(renderedRoute => this._options.postProcess(renderedRoute))) : renderedRoutes) // Check to ensure postProcess hooks returned the renderedRoute object properly.
    .then(renderedRoutes => {
        ...
        return mkdirp(path.dirname(processedRoute.outputPath)).then(() => {
          return new Promise((resolve, reject) => {
            compilerFS.writeFile(processedRoute.outputPath, processedRoute.html.trim(), err => {
              if (err) // eslint-disable-next-line prefer-promise-reject-errors
