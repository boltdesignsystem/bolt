 ...
 * @param {string} path
 */
export function loadCSS(path, callback) {
...
export function injectStyle(style) {
  ...
  if (hasNativeCSSProperties()) {
    ...
    const styleTag = document.getElementById('themify');
    if (!styleTag) {
      inject();
    } else {
 ...
 * @returns {string}
 */
export function _generateNewVariables(customTheme) {
  ...
  return variations.reduce((finalOutput, variation) => {
    ...
    // eslint-disable-next-line
    return (finalOutput += `${
      variation === 'light' ? ':root' : '.' + variation
    }{${variationOutput}}`);
  }, '');
}
 ...
 */
export function loadCSSVariablesFallback(path, cb) {
  if (!hasNativeCSSProperties()) {
    loadCSS(path, cb);
  }
}
...
export function _handleUnSupportedBrowsers(customTheme, palette, JSONFallback) {
  ...
  let finalOutput = Object.keys(customTheme).reduce((acc, variation) => {
    let value = JSONFallback[variation].replace(
      themifyRegExp,
      (occurrence, value) => {
        const [variation, variable, opacity] = value
          .replace(/\s/g, '')
          .split(',');
