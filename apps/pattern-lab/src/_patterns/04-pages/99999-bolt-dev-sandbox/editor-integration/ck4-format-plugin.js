...
CKEDITOR.plugins.add('format-custom', {
  requires: 'richcombo',
  ...
  init: function(editor) {
    ...
    for (var i = 0; i < tags.length; i++) {
      var tag = tags[i];
      // @EDIT: formatting tag name here, try to handle custom-elements
      var formatted_tag = tag.replace('-', '_');
      var style = new CKEDITOR.style(config['format_' + formatted_tag]);
      if (!editor.filter.customConfig || editor.filter.check(style)) {
        stylesCount++;
        styles[formatted_tag] = style;
        styles[formatted_tag]._.enterMode = editor.config.enterMode;
