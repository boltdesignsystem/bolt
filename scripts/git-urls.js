...
function urlCallback(tagUrls) {
  ...
  const latestReleaseLabel = tagUrls[0].label + ' (Last Release)';
  const latestReleaseUrl = 'boltdesignsystem.com';

  tagUrls[0].selected = true;
  tagUrls[0].label = latestReleaseLabel;
  tagUrls[0].label = latestReleaseLabel;

  console.log(tagUrls);

  fs.writeFile(
    process.cwd() + '/www/build/data/bolt-releases.bolt.json',
    JSON.stringify({
      options: tagUrls,
    }),
    'utf8',
...
async function init() {
  try {
    ...
    tags.forEach((tag, index, array) => {
      ...
      urlExists(`https://${tagString}.boltdesignsystem.com`, function(
        err,
        exists,
      ) {
        if (exists === true) {
          tagUrls.push({
            label: tag,
            type: 'option',
            value: `https://${tagString}.boltdesignsystem.com`,
          });
          ...
          urlExists(`https://${tagString}.bolt-design-system.com`, function(
            err,
            exists,
          ) {
            if (exists) {
              tagUrls.push({
                label: tag,
                type: 'option',
                value: `https://${tagString}.bolt-design-system.com`,
              });
            }
